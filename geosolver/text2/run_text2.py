...
def test_trans():
    ...
    for pk, question in questions.iteritems():
        ...
        for number, sentence_words in question.sentence_words.iteritems():
            ...
            grounded_formulas = [ground_formula_node(match_parse, formula) for formula in completed_formulas]
            text_formulas = filter_formulas(flatten_formulas(grounded_formulas))
            all_formulas.extend(expr_formulas + text_formulas)

        result = solve(all_formulas, choice_formulas)
        print result
