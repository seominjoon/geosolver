...
def construct_semantic_forest(semantic_nodes, syntax_score_function, ontology_score_function, semantic_score_function):
    ...
    for from_node, to_node in itertools.product(semantic_nodes.values(), repeat=2):
        score = score_function(from_node, to_node)
        if score > 0:
            edge_scores[(from_node.name, to_node.name)] = score
            forest_graph.add_edge(from_node.name, to_node.name, score=score)
